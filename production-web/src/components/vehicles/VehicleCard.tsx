...
export function VehicleCard({ vehicle, offer, profileOffers, compared = false, onCompare }: { vehicle: Vehicle; offer: Offer; profileOffers?: Offer[]; compared?: boolean; onCompare?: (vehicle: Vehicle) => void }) {
  const profiles = profileOffers?.length ? profileOffers : [offer];
    ...
    <article className="group overflow-hidden rounded-xl border border-line bg-surface shadow-card transition-shadow hover:shadow-lg">
      <div className="relative aspect-[16/10] overflow-hidden bg-slate-100">
        ...
        <div className="absolute top-3 left-3 flex gap-2">
          ...
          <Badge>{vehicle.label}</Badge><Badge>{grouped ? `${profiles.length} perfiles` : audienceLabel[offer.audience]}</Badge>
        </div>
      ...
      <div className="p-5">
        ...
        {grouped ? <div className="mt-5 overflow-hidden rounded-lg border border-line bg-slate-50"><p className="border-b border-line px-3 py-2 text-[0.625rem] font-bold tracking-wide text-muted uppercase">Cuota más baja por perfil</p>{profiles.map((profile) => <Link key={profile.audience} href={`${vehiclePublicPath(vehicle)}?publico=${profile.audience}`} className="flex items-center justify-between gap-4 border-b border-line px-3 py-2.5 last:border-b-0 hover:bg-white"><span className="text-xs font-bold text-copy">{audienceLabel[profile.audience]}</span><span className="text-right"><strong className="font-data text-sm text-ink">{profile.monthlyPrice.toLocaleString("es-ES", { maximumFractionDigits: 2 })} €/mes</strong><small className="ml-1 text-[0.5625rem] font-bold text-muted">{profile.priceIncludesVat ? "IVA incl." : "+ IVA"}</small></span></Link>)}</div> : <div className="mt-5 flex items-end justify-between gap-4">
          <div>
            <p className="text-[0.625rem] font-bold tracking-wide text-muted uppercase">Desde</p>
            <p className="font-data mt-1 text-3xl font-semibold tracking-[-0.06em] text-ink tabular-nums">{offer.monthlyPrice.toLocaleString("es-ES", { maximumFractionDigits: 2 })} €<span className="font-sans text-xs font-semibold tracking-normal text-muted">/mes</span></p>
            <p className="mt-1 text-[0.625rem] font-bold text-muted">{offer.priceIncludesVat ? "IVA incluido" : "+ IVA"}</p>
          </div>
        ...
        <Link href={`${vehiclePublicPath(vehicle)}?publico=${offer.audience}`} className="mt-5 flex min-h-11 items-center justify-between rounded-lg bg-ink px-4 text-sm font-bold text-white transition-colors hover:bg-copy">
          {grouped ? "Ver opciones y condiciones" : "Ver oferta y condiciones"} <ArrowRight size={16} aria-hidden="true" />
        </Link>
      </div>
    </article>
  );
