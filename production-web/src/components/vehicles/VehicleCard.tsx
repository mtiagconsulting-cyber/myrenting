  ...
  return (
    <article className="group flex h-full flex-col overflow-hidden rounded-xl border border-line bg-surface shadow-card transition-shadow hover:shadow-lg">
      ...
      <div className="flex flex-1 flex-col p-5">
        <div className="flex h-5 items-center justify-between gap-3"><p className="truncate text-xs font-bold text-muted">{vehicle.brand}</p>{offer.verifiedAt ? <time dateTime={offer.verifiedAt} className="shrink-0 text-[0.625rem] font-semibold text-positive">Actualizada {new Intl.DateTimeFormat("es-ES", { day: "numeric", month: "short" }).format(new Date(offer.verifiedAt))}</time> : <span className="shrink-0 text-[0.625rem] font-semibold text-amber-700">Confirmar vigencia</span>}</div>
        <h2 className="font-display mt-1 h-8 truncate text-2xl font-semibold tracking-[-0.04em] text-ink">{vehicle.model}</h2>
        <p className="mt-1 h-10 overflow-hidden text-xs leading-5 text-muted">{vehicle.version}</p>
        ...
        <div className="mt-5 flex min-h-24 items-end justify-between gap-4">
