...
import { PriorityArticleContent, priorityArticleFaqs, priorityArticleMetadata } from "@/components/editorial/PriorityArticleContent";
...
export default async function LegacyArticlePage({ params }: Props) {
  ...
  if (!article) notFound();
  const priority = priorityArticleMetadata(slug);
  const priorityFaqs = priorityArticleFaqs(slug);
  return <main id="contenido-principal">
    <Schema data={[editorialSchema({ path: `/blog/${slug}`, title: priority?.title ?? article.headline, description: priority?.description ?? article.description, datePublished: legacy.generatedAt, dateModified: priority ? inventoryUpdatedAt : legacy.generatedAt }), ...(priorityFaqs.length ? [faqSchema(priorityFaqs)] : [])]} />
