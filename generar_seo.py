...
def generar_pagina(make, model, ofertas_modelo):
    ...
    title = f"Renting {nombre} 2026 — Desde {precio_min}€/mes | {g_corto} | MiRenting"
    description = f"Compara el renting del {nombre} en {', '.join(gestoras)}. Precio desde {precio_min}€/mes. {len(ofertas_modelo)} ofertas con seguro y mantenimiento incluidos. Sin formularios ni registros."
    ...
    breadcrumb_schema = json.dumps({"@context":"https://schema.org","@type":"BreadcrumbList","itemListElement":[{"@type":"ListItem","position":1,"name":"MiRenting","item":"https://www.myrenting.es/"},{"@type":"ListItem","position":2,"name":f"Renting {nombre}","item":f"https://www.myrenting.es/{slug_name}.html"}]}, ensure_ascii=False)
    product_schema = json.dumps({"@context":"https://schema.org","@type":"Product","name":f"Renting {nombre}","description":description,"offers":{"@type":"AggregateOffer","lowPrice":str(precio_min),"highPrice":str(precio_max),"priceCurrency":"EUR","offerCount":str(len(ofertas_modelo))}}, ensure_ascii=False)
